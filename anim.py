...
import RLmap
import RLobject
from RLCONSTANTS import WWIDTH, WHEIGHT, IMGSIZE, IMGDIR, RESDIR, DARKFLOOR, images
...
def intro(game):
    ...
    select_pos = [(WWIDTH / 2 - 10, WHEIGHT / 3 + 138), (WWIDTH / 2 - 10, WHEIGHT / 3 + 156),
                  (WWIDTH / 2 - 10, WHEIGHT / 3 + 174)]
    pos = 0
...
def displayInstructions(game):
    game.surface.fill((0, 0, 0))
    f = open(os.path.join(RESDIR, 'tutorial_text.txt'), 'r')
    lines = f.readlines()
